    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6 sm:mb-8">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-[#002045]">Doctors Directory</h1>
                        ...
                        <thead>
                            <tr className="bg-slate-50">
                                {['Doctor', 'Specialization', 'Contact', 'Status', ''].map((h) => (
                                    <th key={h} className={`px-4 sm:px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider ${h === '' ? 'text-right' : ''}`}>
                                        {h}
                                    </th>
                                ))}
                            </tr>
                                        ...
                                        <tr key={id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-4 sm:px-6 py-4 text-right">
                                                <div className="flex justify-end gap-1 sm:gap-2">
                                                    <button onClick={() => navigate(`/admin/doctors/${id}/schedule`)} className="p-1.5 sm:p-2 hover:bg-blue-50 hover:text-[#002045] rounded-lg text-slate-400 transition-colors" title="Manage Schedule">
                                                        <span className="material-symbols-outlined text-lg">calendar_month</span>
                                                    </button>
                                                    <button onClick={() => toggleDoctor(id)} className="p-1.5 sm:p-2 hover:bg-slate-100 rounded-lg text-slate-400 transition-colors" title={isActive ? 'Deactivate' : 'Activate'}>
