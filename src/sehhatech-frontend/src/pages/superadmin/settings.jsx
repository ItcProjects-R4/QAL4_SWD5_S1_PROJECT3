  ...
  return (
    <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className={`w-11 h-6 bg-slate-200 rounded-full peer
        ...
        after:h-5 after:w-5 after:transition-all peer-checked:${activeColor}`} />
    </label>
  );
}
...
export default function Settings() {
  ...
  // Load profile
  useEffect(() => {
    apiFetch("/api/superadmin/profile")
      .then((d) => setProfile({ name: d.name ?? "Super Admin", role: d.role ?? "Super Administrator", email: d.email ?? "—" }))
      .catch(() => {});
  }, []);
